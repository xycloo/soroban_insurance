...
pub fn compute_fee_per_share(
    ...
    let interest_by_supply = accrued_interest
...
pub(crate) fn calculate_to_mint(
    e: &Env,
    amount: i128,
    total_supply: i128,
    ...
    let multiplier = total_supply
        .fixed_div_floor(total_liquidity, STROOP.into())
        .unwrap();

    amount.fixed_mul_floor(multiplier, STROOP.into()).unwrap()
}

pub(crate) fn calculate_principal_value(
    shares: i128,
    tot_liquidity: i128,
    tot_shares: i128,
) -> i128 {
    let multiplier = tot_liquidity
        .fixed_div_floor(tot_shares, STROOP.into())
        .unwrap();

    shares.fixed_mul_floor(multiplier, STROOP.into()).unwrap()
}
...
pub(crate) fn calculate_refund(time_to_end: i128, amount: i128, multiplier: i128) -> i128 {
    let coefficient = amount * multiplier / time_to_end;
